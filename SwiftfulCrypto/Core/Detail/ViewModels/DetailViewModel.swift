...
class DetailViewModel: ObservableObject {
    ...
    private func createAdditionalArray(coinDetailModel: CoinDetailModel?, coinModel: CoinModel) -> [StatisticModel] {
        ...
        let marketCapChange = "$" + (
            coinModel.marketCapChange24H?
                .formattedWithAbbreviations() ?? ""
         )
        let marketCapPercentChange = coinModel.marketCapChange24H
        let marketCapChangeStat = StatisticModel(title: "24h Market Cap changechanged", value: marketCapChange, percentageChange: marketCapPercentChange)
